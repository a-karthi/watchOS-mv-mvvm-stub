...
class NetworkManager {
    ...
    private static func setupURLPath(request: NetworkRequest) -> URL {
        ...
        case .directURL:
            let fullPath = String(format: "%@%@", SampleTV.baseURL as CVarArg, request.path)
            let fullPathSTR = fullPath.replacingOccurrences(of: "%20+0000", with: "")
            return URL(string: fullPathSTR)!

        default:
            return request.constructURL(baseURL: SampleTV.baseURL, additionalURLQueryItems: [])
    ...
    static func execute<T: Decodable>(request: NetworkRequest,
        ...
        #if os(iOS)
        print("iOS")
        if !iOSReachability.isConnectedToNetwork() {
